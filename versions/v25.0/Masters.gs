 ...
 */
function getStaffSettings(email) {
  try {
    const props = PropertiesService.getUserProperties();
    const key = `settings_${email.replace('@', '_at_')}`;
    ...
    return createErrorResponse('設定の取得に失敗しました: ' + e.message);
  }
}
 ...
 */
function updateStaffSettings(email, settings) {
  try {
    const props = PropertiesService.getUserProperties();
    const key = `settings_${email.replace('@', '_at_')}`;
    ...
    console.log('設定保存:', email, merged);
    return createSuccessResponse({ saved: true });
  } catch (e) {
    console.error('設定保存エラー:', e);
    return createErrorResponse('設定の保存に失敗しました: ' + e.message);
  }
}
